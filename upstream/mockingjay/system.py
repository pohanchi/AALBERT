...
from .model import Mockingjay, SpecHead
...
class PretrainedSystem(pl.LightningModule):
    ...
    def __init__(self, args, model_config, training_config, **kwargs):
        ...
        tradition_feat = getattr(importlib.import_module('hubconf'),training_config['datarc']['input']['feature_type'])
        self.tradition_feat_extractor = tradition_feat(config=training_config['datarc']['input']['config_path'])
        self.masking_strategy = training_config['masking_strategy']
        self.pretrained_model = Mockingjay(model_config['model'])
        ...
        for feat in training_config['datarc']['target']:
            ...
            **model_config['model']['transform'], "output_dim": tradition_feat_system.get_output_dim(), "name": feat['feature_type']}
    ...
    def downsample(self, feats):
        ...
        left_feats = []
        for feat in feats:
            length_after_downsample = (feat.shape[0] // self.downsample_rate) * self.downsample_rate
            left_feat = feat[:length_after_downsample]
            stack_feat = left_feat.reshape(length_after_downsample // self.downsample_rate, -1)
    ...
    def generate_mask_frame(self, stack_feats):
        ...
        for stack in stack_feats:
            ...
            valid_box_size = (len(stack) // self.masking_strategy['mask_consecutive'])
            remainder = len(stack) % self.masking_strategy['mask_consecutive']
            probs = torch.zeros(valid_box_size).fill_(self.masking_strategy['mask_proportion']/self.masking_strategy['mask_consecutive']) 
            
            # generate start point to mask frame
            start_indexes = torch.nonzero(torch.bernoulli(probs))
            offset = random.randint(0,remainder)
            base_indexes = start_indexes.expand(start_indexes.size(0), self.masking_strategy['mask_consecutive'])
            consecutive_offset = torch.arange(self.masking_strategy['mask_consecutive']).long().expand(base_indexes.size(0), self.masking_strategy['mask_consecutive'])
    ...
    def training_step(self, batch, batch_idx):
        ...
        input_feats = pad_sequence(stack_feats, batch_first=True)
        mask_indexes = pad_sequence(mask_indexes, batch_first=True)[:,:,None].to(self.device).bool()
        input_att_masks = pad_sequence(att_masks, batch_first=True).to(self.device)
        input_pos_idxes = self.generate_pos_idxes(input_att_masks).to(self.device)
        forward_config = {"spec_input": input_feats, "att_mask": input_att_masks, "pos_idx": input_pos_idxes, "layer_index":-1}
        ...
        for pred_head_idx in range(len(self.pretrained_heads)):
            ...
            reconstruct_feats, _ = self.pretrained_heads[pred_head_idx](last_hidden_states)
            masked_spec_loss = self.objective_loss(reconstruct_feats.masked_select(mask_indexes), ds_feats.masked_select(mask_indexes)) / torch.sum(mask_indexes) / self.downsample_rate
    ...
    def configure_optimizers(self):
        ...
        pretrained_model_decay_parameters = [p for n, p in self.pretrained_model.named_parameters() if not any(nd in n for nd in no_decay)]
        pretrained_heads_decay_parameters = [p for head in self.pretrained_heads for n, p in head.named_parameters() if not any(nd in n for nd in no_decay)]

        pretrained_model_nodecay_parameters = [p for n, p in self.pretrained_model.named_parameters() if any(nd in n for nd in no_decay)]
        pretrained_heads_nodecay_parameters = [p for head in self.pretrained_heads for n, p in head.named_parameters() if any(nd in n for nd in no_decay)]
        ...
        optimizer = eval(f"torch.optim.{optimizer_config.pop('name')}")(params=all_parameters, **optimizer_config)

        if self.scheduler_config:
            scheduler_config = copy.deepcopy(self.scheduler_config)
            scheduler = eval(f"get_{scheduler_config.pop('name')}")(optimizer=optimizer, **scheduler_config)
